...
if __name__ == "__main__":
    ...
    job_ids_formatted = ", ".join([f'"{id_}"' for id_ in list(job_ads.job_id)])
    ...
    job_titles_salary_df = pd.read_sql(query_job_titles_salary, conn).rename(
        columns={"id": "job_id"}
    )
    ...
    clean_salaries_df = pd.read_sql(query_clean_salary, conn)
    logger.info("got annualised clean salaries from sql database...")

    # merge all 3 dataframes
    all_dfs = [job_ads, job_titles_salary_df, clean_salaries_df]
    job_ads_with_metadata = reduce(
        lambda left, right: pd.merge(left, right, on="job_id", how="outer"), all_dfs
    )
    logger.info("merged deduplicated job data with additional metadata...")

    file_name_with_metadata = os.path.join(
        s3_folder, "dedupe_analysis_metadata_salaries_titles.csv"
    )
