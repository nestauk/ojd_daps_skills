Search.setIndex({
  docnames: ["index", "labelling", "model_card", "pipeline_summary"],
  filenames: [
    "index.rst",
    "labelling.md",
    "model_card.md",
    "pipeline_summary.md",
  ],
  titles: [
    "&lt;no title&gt;",
    "Entity Labelling",
    "Model Cards",
    "Pipeline summary",
  ],
  terms: {
    skill: [0, 1],
    extractor: [0, 3],
    welcom: 0,
    nesta: [0, 2],
    "": [0, 2],
    librari: [0, 2],
    instal: 0,
    aw: 0,
    cli: 0,
    tl: 0,
    dr: 0,
    us: [0, 1, 2],
    1: [0, 2],
    extract: [0, 1, 2],
    AND: 0,
    map: 0,
    2: [0, 2],
    3: [0, 1, 3],
    app: 0,
    develop: [0, 2],
    setup: 0,
    project: 0,
    structur: 0,
    test: [0, 1, 2],
    contributor: 0,
    guidelin: 0,
    custom: 0,
    usag: 0,
    configur: [0, 2],
    file: [0, 2],
    predefin: [0, 2],
    definit: [0, 2],
    your: 0,
    own: 0,
    taxonomi: [0, 3],
    format: 0,
    defin: [0, 1],
    pipelin: [0, 1, 2],
    summari: 0,
    intend: 0,
    out: [0, 1, 2],
    scope: 0,
    metric: 0,
    comparison: 0,
    top: 0,
    group: [0, 2],
    per: [0, 2],
    occup: 0,
    esco: [0, 2],
    essenti: 0,
    degre: 0,
    overlap: 0,
    between: [0, 2],
    lightcast: 0,
    our: [0, 1],
    evalu: [0, 1, 2],
    manual: [0, 2],
    judgement: 0,
    fals: 0,
    posit: 0,
    rate: 0,
    qualiti: 0,
    model: [0, 1, 3],
    card: 0,
    name: [0, 1, 3],
    entiti: [0, 3],
    recognit: [0, 1, 3],
    train: [0, 3],
    ner: [0, 1, 3],
    multiskil: [0, 1, 3],
    caveat: 0,
    recommend: 0,
    factor: 0,
    label: [0, 2],
    dataset: [0, 3],
    The: [0, 1, 2, 3],
    extractskil: 0,
    class: 0,
    load: 0,
    extract_skil: 0,
    get_skil: 0,
    map_skil: 0,
    format_skil: 0,
    mit: 0,
    licens: 0,
    To: 1,
    from: [1, 2, 3],
    job: [1, 2, 3],
    advert: [1, 2, 3],
    we: [1, 2, 3],
    took: 1,
    an: [1, 2, 3],
    approach: [1, 2, 3],
    predict: [1, 2, 3],
    which: [1, 3],
    part: [1, 2],
    were: [1, 2, 3],
    experi: [1, 2, 3],
    need: [1, 2],
    data: [1, 2],
    first: [1, 2],
    creat: 1,
    random: [1, 3],
    sampl: [1, 2, 3],
    got: 1,
    them: [1, 2],
    form: 1,
    studio: 1,
    more: [1, 2, 3],
    about: [1, 2],
    thi: [1, 2, 3],
    process: 1,
    can: [1, 2],
    found: [1, 2, 3],
    skill_ner: 1,
    There: [1, 3],
    ar: [1, 2, 3],
    user: [1, 3],
    interfac: 1,
    task: 1,
    look: [1, 2, 3],
    like: 1,
    tri: 1,
    best: [1, 3],
    start: 1,
    end: 1,
    each: 1,
    individu: [1, 3],
    verb: 1,
    given: [1, 3],
    sometim: 1,
    wasn: 1,
    t: [1, 3],
    easi: 1,
    exampl: [1, 2],
    earlier: 1,
    sentenc: [1, 2, 3],
    might: [1, 2],
    later: 1,
    i: [1, 2, 3],
    work: [1, 2, 3],
    team: 1,
    basi: 1,
    could: [1, 2],
    singl: [1, 3],
    make: 1,
    sens: 1,
    without: [1, 3],
    word: [1, 2],
    In: 1,
    situat: 1,
    whole: 1,
    span: [1, 2],
    multi: [1, 2, 3],
    often: 1,
    follow: [1, 2],
    e: [1, 2, 3],
    g: [1, 2],
    insur: 1,
    includ: [1, 3],
    some: 1,
    qualif: 1,
    electr: 1,
    For: [1, 2, 3],
    current: [1, 2, 3],
    5641: [1, 2],
    375: [1, 2],
    354: [1, 2],
    4696: [1, 2],
    608: [1, 2],
    20: [1, 2],
    held: [1, 2],
    set: [1, 2, 3],
    page: 2,
    contain: [2, 3],
    inform: [2, 3],
    differ: [2, 3],
    detail: 2,
    two: [2, 3],
    main: 2,
    scientist: 2,
    analyt: 2,
    practic: [2, 3],
    last: 2,
    updat: 2,
    23: 2,
    11: [2, 3],
    2022: 2,
    spaci: 2,
    compon: 2,
    whether: [2, 3],
    scikit: 2,
    learn: 2,
    svm: [2, 3],
    featur: 2,
    length: 2,
    split: [2, 3],
    where: [2, 3],
    possibl: 2,
    base: 2,
    semant: 2,
    rule: [2, 3],
    As: 2,
    "15th": 2,
    novemb: 2,
    ojo: 2,
    neural: 2,
    network: 2,
    architectur: 2,
    Their: 2,
    sophist: 2,
    embed: [2, 3],
    strategi: 2,
    subword: 2,
    bloom: 2,
    deep: 2,
    convolut: 2,
    residu: 2,
    connect: 2,
    novel: 2,
    transit: 2,
    pars: 2,
    here: 2,
    you: 2,
    read: 2,
    creation: 2,
    A: 2,
    python: 2,
    nerevalu: 2,
    wa: 2,
    calcul: [2, 3],
    f1: 2,
    precis: 2,
    recal: 2,
    classifi: 2,
    result: [2, 3],
    0: [2, 3],
    586: 2,
    679: 2,
    515: 2,
    506: 2,
    648: 2,
    416: 2,
    all: [2, 3],
    563: 2,
    643: 2,
    500: 2,
    These: 2,
    partial: 2,
    match: [2, 3],
    perform: [2, 3],
    across: 2,
    both: [2, 3],
    output: 2,
    ner_model: 2,
    20220825: 2,
    train_detail: 2,
    json: 2,
    same: 2,
    On: [2, 3],
    achiev: 2,
    91: 2,
    accuraci: 2,
    when: [2, 3],
    splitter: 2,
    algorithm: [2, 3],
    253: 2,
    good: 2,
    ok: [2, 3],
    bad: 2,
    Of: [2, 3],
    80: 2,
    66: 2,
    9: [2, 3],
    25: 2,
    take: 2,
    mani: [2, 3],
    do: [2, 3],
    get: 2,
    If: 2,
    unabl: 2,
    still: 2,
    choic: [2, 3],
    futur: 2,
    should: [2, 3],
    add: 2,
    dedupl: [2, 3],
    mean: 2,
    mention: 2,
    excel: [2, 3],
    twice: 2,
    just: 2,
    onc: 2,
    howev: [2, 3],
    string: 2,
    slightli: 2,
    occurr: 2,
    specif: 2,
    text: [2, 3],
    disambigu: 2,
    acronym: 2,
    improv: 2,
    methodologi: 2,
    threshold: [2, 3],
    number: [2, 3],
    chang: 2,
    config: 2,
    similar: [2, 3],
    try: 2,
    most: 2,
    granular: 2,
    level: [2, 3],
    cosin: [2, 3],
    huggingfac: [2, 3],
    transform: [2, 3],
    minilm: [2, 3],
    l6: [2, 3],
    v2: [2, 3],
    close: 2,
    abov: 2,
    7: [2, 3],
    assign: 2,
    one: 2,
    maximum: 2,
    share: 2,
    see: 2,
    diagram: 2,
    43: 2,
    commonli: 2,
    occur: 2,
    100: [2, 3],
    "000": [2, 3],
    hard: 2,
    code: 2,
    step: 2,
    doe: 2,
    less: 2,
    well: [2, 3],
    metaphor: 2,
    understand: [2, 3],
    bigger: 2,
    pictur: 2,
    draft: 2,
    orsa: 2,
    fine: 2,
    art: 2,
    poor: 2,
    assist: [2, 3],
    provid: 2,
    gener: 2,
    peopl: 2,
    import: 2,
    ensur: 2,
    everi: 2,
    highest: 2,
    unmatch: 2,
    under: 2,
    identifi: [2, 3],
    approxim: 2,
    relev: 2,
    tune: 2,
    high: 3,
    overal: 3,
    support: 3,
    vector: 3,
    machin: 3,
    appli: 3,
    emb: 3,
    pre: 3,
    onto: 3,
    further: 3,
    feedback: 3,
    pleas: 3,
    contact: 3,
    liz: 3,
    gallagh: 3,
    india: 3,
    kerl: 3,
    cath: 3,
    sleeman: 3,
    onlin: 3,
    european: 3,
    commiss: 3,
    compet: 3,
    open: 3,
    research: 3,
    labour: 3,
    statist: 3,
    relat: 3,
    govern: 3,
    bodi: 3,
    non: 3,
    english: 3,
    languag: 3,
    other: 3,
    than: 3,
    draw: 3,
    conclus: 3,
    new: 3,
    unidentifi: 3,
    determin: 3,
    demand: 3,
    expert: 3,
    steer: 3,
    input: 3,
    nor: 3,
    ani: 3,
    discriminatori: 3,
    hire: 3,
    exact: 3,
    wai: 3,
    how: 3,
    have: 3,
    sever: 3,
    proxi: 3,
    better: 3,
    compar: 3,
    also: 3,
    least: 3,
    onli: 3,
    75: 3,
    percentil: 3,
    count: 3,
    At: 3,
    find: 3,
    58: 3,
    averag: 3,
    345: 3,
    54: 3,
    transvers: 3,
    addit: 3,
    94: 3,
    5: 3,
    percent: 3,
    median: 3,
    97: 3,
    84: 3,
    99: 3,
    minimum: 3,
    guarante: 3,
    10: 3,
    22: 3,
    while: 3,
    6: 3,
    42: 3,
    40: 3,
    39: 3,
    present: 3,
    percentag: 3,
    33: 3,
    qualit: 3,
    limit: 3,
    list: 3,
    mai: 3,
    veri: 3,
    financi: 3,
    account: 3,
    v: 3,
    financ: 3,
    consid: 3,
    would: 3,
    otherwis: 3,
    so: 3,
    allow: 3,
    inappropri: 3,
    ey: 3,
    examin: 3,
    suppli: 3,
    chain: 3,
    role: 3,
    sourc: 3,
    truth: 3,
    floor: 3,
    care: 3,
    64: 3,
    judg: 3,
    shouldn: 3,
    been: 3,
    analysi: 3,
    api: 3,
    show: 3,
    27: 3,
    12: 3,
    tag: 3,
    thought: 3,
    thei: 3,
    had: 3,
    felt: 3,
    183: 3,
    73: 3,
    19: 3,
    8: 3,
    172: 3,
    53: 3,
    30: 3,
    17: 3,
    16: 3,
    35: 3,
    83: 3,
    87: 3,
    either: 3,
    lowest: 3,
    its: 3,
    score: 3,
    71: 3,
    hierarchi: 3,
    51: 3,
    90: 3,
    attitud: 3,
    63: 3,
    knoweldg: 3,
    "00": 3,
  },
  objects: {},
  objtypes: {},
  objnames: {},
  titleterms: {
    entiti: [1, 2],
    label: 1,
    train: [1, 2],
    dataset: 1,
    model: 2,
    card: 2,
    name: 2,
    recognit: 2,
    extract_skills_card: 2,
    summari: [2, 3],
    ner: 2,
    metric: [2, 3],
    multiskil: 2,
    caveat: 2,
    recommend: 2,
    skill: [2, 3],
    taxonomi: 2,
    map: [2, 3],
    mapping_card: 2,
    factor: 2,
    pipelin: 3,
    intend: 3,
    us: 3,
    out: 3,
    scope: 3,
    comparison: 3,
    1: 3,
    top: 3,
    group: 3,
    per: 3,
    occup: 3,
    esco: 3,
    essenti: 3,
    2: 3,
    degre: 3,
    overlap: 3,
    between: 3,
    lightcast: 3,
    "": 3,
    extract: 3,
    our: 3,
    evalu: 3,
    manual: 3,
    judgement: 3,
    fals: 3,
    posit: 3,
    rate: 3,
    qualiti: 3,
  },
  envversion: {
    "sphinx.domains.c": 2,
    "sphinx.domains.changeset": 1,
    "sphinx.domains.citation": 1,
    "sphinx.domains.cpp": 8,
    "sphinx.domains.index": 1,
    "sphinx.domains.javascript": 2,
    "sphinx.domains.math": 2,
    "sphinx.domains.python": 3,
    "sphinx.domains.rst": 2,
    "sphinx.domains.std": 2,
    "sphinx.ext.viewcode": 1,
    sphinx: 57,
  },
  alltitles: {
    "Entity Labelling": [[1, "entity-labelling"]],
    "Training dataset": [[1, "training-dataset"]],
    "Model Cards": [[2, "model-cards"]],
    'Model Card: Named Entity Recognition Model <a name="extract_skills_card"></a>': [
      [
        2,
        "model-card-named-entity-recognition-model-a-name-extract-skills-card-a",
      ],
    ],
    Summary: [
      [2, "summary"],
      [2, "id1"],
    ],
    Training: [[2, "training"]],
    "NER Metrics": [[2, "ner-metrics"]],
    "Multiskill Metrics": [[2, "multiskill-metrics"]],
    "Caveats and Recommendations": [
      [2, "caveats-and-recommendations"],
      [2, "id2"],
    ],
    'Model Card: Skills to Taxonomy Mapping <a name="mapping_card"></a>': [
      [2, "model-card-skills-to-taxonomy-mapping-a-name-mapping-card-a"],
    ],
    "Model Factors": [[2, "model-factors"]],
    "Pipeline summary": [[3, "pipeline-summary"]],
    "Intended Use": [[3, "intended-use"]],
    "Out of Scope Uses": [[3, "out-of-scope-uses"]],
    Metrics: [[3, "metrics"]],
    "Comparison 1 - Top skill groups per occupation comparison to ESCO essential skill groups per occupation": [
      [
        3,
        "comparison-1-top-skill-groups-per-occupation-comparison-to-esco-essential-skill-groups-per-occupation",
      ],
    ],
    "Comparison 2 - Degree of overlap between Lightcast\u2019s extracted skills and our Lightcast skills": [
      [
        3,
        "comparison-2-degree-of-overlap-between-lightcasts-extracted-skills-and-our-lightcast-skills",
      ],
    ],
    "Evaluation 1 - Manual judgement of false positive rate": [
      [3, "evaluation-1-manual-judgement-of-false-positive-rate"],
    ],
    "Evaluation 2 - Manual judgement of skills extraction and mapping quality": [
      [
        3,
        "evaluation-2-manual-judgement-of-skills-extraction-and-mapping-quality",
      ],
    ],
  },
  indexentries: {},
});
